...
function buildReceiptLines(products, offers) {
  ...
  for (const item of products) {
    ...
    lines.push({ product: p, quantity: item.quantity, isFree: false });
    const offer = offers.find((o) => o.active && o.category === p.category);
    if (offer) {
      const freeQty = Math.floor(item.quantity / offer.buyQty) * offer.freeQty;
      if (freeQty > 0) {
        lines.push({
          product: { name: `${p.name} (offer bonus)`, price: 0 },
          quantity: freeQty,
          isFree: true,
        });
      }
...
async function sendConfirmationEmail(order, receiptLines) {
  ...
  const productRows = receiptLines.map((line, i) => {
    ...
    return `<tr>
      ...
      <td style="padding:8px;border-bottom:1px solid #E8DDD9;text-align:right;">${line.isFree ? '<strong style="color:#8C5E6B;">FREE</strong>' : `${price.toFixed(2)} lei`}</td>
      <td style="padding:8px;border-bottom:1px solid #E8DDD9;text-align:right;">${line.isFree ? '<strong style="color:#8C5E6B;">0.00 lei</strong>' : `${lineTotal.toFixed(2)} lei`}</td>
    </tr>`;
  }).join('');
  ...
  const payload = {
    Messages: [
      {
        From: { Email: 'maracosmetics12@gmail.com', Name: 'Velvet Rose' },
